...
$shareCode = $hFunction->getTimeCode() . $bannerId;
...
@section('tf_main_action_content')
    <form id="frmMapBannerShare" name="frmMapBannerShare"
          class="panel panel-default tf-border-none tf-padding-none tf-margin-bot-none "
          method="post" action="{!! route('tf.map.banner.share.post', $bannerId) !!}">
        <div class="panel-heading tf-bg tf-color-white tf-border-none ">
            <i class="tf-font-size-16 fa fa-share-alt"></i>&nbsp;
            {!! trans('frontend_map.banner_share_title') !!}
        </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <ul class="list-group ">
                        <li class="list-group-item tf-border-none tf-padding-none ">
                            <label>{!! trans('frontend_map.banner_share_friend_label') !!}:</label>
                        </li>
                        <li class="list-group-item tf-margin-padding-none tf-border-none">
                            <input id="tf_banner_share_select_all"
                                   type="checkbox">&nbsp;{!! trans('frontend_map.banner_share_friend_all_label') !!}
                        </li>
                        <li class="list-group-item tf-margin-padding-none tf-border-none">
                    ...
                    <div class="form-group">
                        <label>{!! trans('frontend_map.banner_share_email_label') !!}</label>
                        <input class="form-control" name="txtEmail" type="text"
                               placeholder="{!! trans('frontend_map.banner_share_email_placeholder') !!}">
                    </div>
                    <div class="form-group">
                        <label>{!! trans('frontend_map.banner_share_message_label') !!}</label>
                    <textarea class="form-control" name="txtMessage" rows="3"
                              placeholder="{!! trans('frontend_map.banner_share_message_placeholder') !!}"></textarea>

                    </div>
                    <div class="form-group text-center">
                        <input type="hidden" name="shareCode" value="{!! $shareCode !!}">
                        <input type="hidden" name="_token" value="{!! csrf_token() !!}"/>
                        <button type="button" class="tf_map_banner_share_send btn btn-primary">
        ...
        <div class="panel panel-footer  text-left tf-margin-bot-none">
            <div class="form-group tf-padding-lef-10 tf-padding-rig-20">
                <label>
                    {!! trans('frontend_map.banner_share_link_label') !!}
                </label>

                <div class="input-group">
                    <input id="tf_banner_share_link" type="text" class="tf_share_link form-control"
                           value="{!! $shareLink !!}"
                           data-href="{!! route('tf.map.banner.share.link', "$bannerId/$shareCode") !!}">
                <span class="input-group-btn">
                    <button class="btn btn-default tf_copy" type="button"
                            title="{!! trans('frontend_map.banner_share_link_copy_title') !!}">
