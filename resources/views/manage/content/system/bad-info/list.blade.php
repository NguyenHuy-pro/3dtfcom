...
$title = 'Bad info';
?>
@extends('manage.content.system.bad-info.index')
    ...
    <div class="row tf-m-c-container-object">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 tf-bg " >
            <div class="col-xs-6 col-sm-6 col-md-6 tf-line-height-40 tf-color-white">
                Total : {!! $modelBadInfo->totalRecords() !!}
            </div>
            <div class="col-xs-6 col-sm-6 col-md-6 tf-line-height-40 text-right">
                @if($actionStatus)
                    <a href="{!! route('tf.m.c.system.badInfo.add.get') !!}">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 tf_list_object"
             data-href-view="{!! route('tf.m.c.system.badInfo.view') !!}"
             data-href-edit="{!! route('tf.m.c.system.badInfo.edit.get') !!}"
             data-href-del="{!! route('tf.m.c.system.badInfo.delete') !!}">
            <table class="table table-hover">
                ...
                @if(count($dataBadInfo) > 0)
                    <?php
                    $perPage = $dataBadInfo->perPage();
                    $currentPage = $dataBadInfo->currentPage();
                    $n_o = ($currentPage == 1) ? 0 : ($currentPage - 1) * $perPage; // set row number
                    ?>
                    @foreach($dataBadInfo as $itemBadInfo)
                        <?php
                        $badInfoId = $itemBadInfo->badInfoId();
                        ?>
                        <tr class="tf_object" data-object="{!! $badInfoId !!}">
