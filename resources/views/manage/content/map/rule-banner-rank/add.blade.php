...
@section('tf_m_c_container_object')
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center">
        <h3 class="tf-margin-30">Add rule of banner rank </h3>
    </div>
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <form class="tf_frm_add col-xs-12 col-md-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2" method="post" role="form"
              action="{!! route('tf.m.c.map.rule_banner_rank.add.post') !!}">
            @if (Session::has('notifyAddRuleBannerRank'))
                <div class="form-group text-center tf-color-red">
                    {!! Session::get('notifyAddRuleBannerRank') !!}
                    <?php
                    Session::forget('notifyAddRuleBannerRank');
                    ?>
                </div>
            @endif
                ...
                <select class="form-control" id="cbSize" name="cbSize">
                    ...
                    @foreach($objectSize as $itemSize)
                        <option value="{!! $itemSize->sizeId() !!}">
                            {!! $itemSize->name() !!}
                            @if($modelRuleBannerRank->existSize($itemSize->sizeId()))
                                &nbsp;&nbsp; <em>(Exist)</em>
                            @endif
                        </option>
                    @endforeach
            ...
            <div class="form-group text-center">
                ...
                <a href="{!! route('tf.m.c.map.rule_banner_rank.list') !!}">
