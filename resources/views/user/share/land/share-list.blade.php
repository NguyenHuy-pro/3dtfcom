...
#share
$shareInfo = $dataUser->infoLandShare($userAccessId, $take, $dateTake);

?>
<div class="row tf_user_share_container" data-user="{!! $userAccessId !!}"
     data-href-view="{!! route('tf.user.share.land.detail') !!}">
    <div class="tf_list_content tf-bg-white col-xs-12 col-sm-12 col-md-12 col-lg-12">
        @if(count($shareInfo) > 0)
            @foreach($shareInfo as $dataLandShare)
                @include('user.share.land.share-object', compact('dataLandShare'),
                    [
                        'modelUser'=>$modelUser,
                        'dataUser'=> $dataUser
                    ])
                <?php
                $checkDateViewMore = $dataLandShare->createdAt();
                ?>
            @endforeach
        @endif
    </div>
    ...
    @if(count($shareInfo) > 0 )
        ...
        @if(count($resultMore) > 0)
            <div class="tf_view_more tf-padding-20 text-center col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <a class="tf-link" data-take="{!! $take !!}" data-href="{!! route('tf.user.share.land.more.get') !!}">
                    {!! trans('frontend_user.share_land_view_more') !!}
                </a>
            </div>
        @endif
    @endif
