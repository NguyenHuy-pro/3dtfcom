...
?>
<div class="row">
    <div id="tf_building_service_article_detail_comment"
         class="tf-building-service-article-detail-comment col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="row">
            <div class="tf-title col-xs-12 col-sm-12 col-md-12 col-lg-12">
                {!! $totalComment !!}
                {!! trans('frontend_building.service_article_detail_comment_title_label') !!}
            </div>
        </div>
        <div class="tf-margin-bot-20 row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <form class="tf_form_comment tf-form form-horizontal col-xs-12 col-sm-12 col-md-12 col-lg-12"
                      role="form"
                      enctype="multipart/form-data" method="post"
                      action="{!! route('tf.building.services.article.detail.comment.add') !!}">
        ...
        <div class="row">
            <div id="tf_building_service_article_detail_comment_list" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"
                 data-href-edit="{!! route('tf.building.services.article.detail.comment.edit.get') !!}"
                 data-href-del="{!! route('tf.building.services.article.detail.comment.delete') !!}">
                @if(count($buildingArticlesComment) > 0)
                    @foreach($buildingArticlesComment as $dataBuildingArticlesComment)
                        @include('building.services.detail.comment.comment-object', compact('modelUser','dataBuildingArticlesComment'))
                    @endforeach
                @endif
            </div>
            @if($totalComment > count($buildingArticlesComment))
                <div id="tf_building_service_article_detail_comment_more"
                     class="tf-view-more col-xs-12 col-sm-12 co-md-12 col-lg-12 ">
                    <a class="tf-link" data-take="{!! $take !!}" data-articles="{!! $articleId !!}"
                       data-href="{!! route('tf.building.services.article.detail.comment.view_more') !!}">
